...
- **[THEORY.md](THEORY.md)**: Formal mathematical proofs, including the closed-form for $N_b(m)$ and torsor counts.
...
## Core Implementation
- **[engine.py](engine.py)**: Optimized Simulated Annealing (SA) engine (v3) using NumPy-accelerated scoring and local successor updates. Supports arbitrary $m$ and $k$.

## Key Results
- **P1-k4 Record:** Score 74 achieved for $m=4, k=4$.
- **P2 Progress:** Score 17 achieved for $m=6, k=3$.
- **$N_b(m)$ Formula:** Derived and verified $N_b(m) = m^{m-1} \cdot \varphi(m)$.
- **Torsor Resolution:** Resolved the $648$ count for $m=3, k=3$ as $162 \times 4$.
